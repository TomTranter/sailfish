...
class ExternalSimulation(LBFluidSim, LBForcedSim):
    ...
    def modify_config(cls, config):
        if not config.geometry:
            return
        config.geometry = io.imread('/home/tom/test_images/'+sample+'.tif')[50:250,50:250,50:250]
        #config.geometry = io.imread('/home/tom/test_images/e25.tif')
    ...
    def _u_mag(self, runner):
        ...
        u_mag = np.sqrt(runner._sim.vx[m]**2 + runner._sim.vy[m]**2 + runner._sim.vz[m]**2)
        return u_mag
...
#                print(fo.force())
    ...
    def __init__(self, config):
        ...
        self.add_body_force((self.F, 0.0, 0.0))
...
if __name__ == '__main__':
    ...
    data = np.load('/home/tom/sailfish/examples/unorm.npz')
    import matplotlib.pyplot as plt
    fig = plt.figure()
    plt.plot(data['it'], data['perm'], 'ro-')
    plt.title('Q: '+Q_factor)
    plt.xlabel('Iterations')
    plt.ylabel('Permeability')
    plt.savefig('/home/tom/sailfish/examples/'+sample+'_perm_'+Q_factor+'.png')
    fig = plt.figure()
    plt.title('Q: '+Q_factor)
    plt.plot(data['it'], data['tau'], 'ro-')
    plt.xlabel('Iterations')
    plt.ylabel('Tortuosity')
    plt.savefig('/home/tom/sailfish/examples/'+sample+'_tau_'+Q_factor+'.png')
